<div class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/40 to-blue-50 text-slate-800 antialiased selection:bg-indigo-500 selection:text-white pb-12">
    
    {{-- Emergency Banner (3D Pulsing Floating Card) --}}
    <div class="max-w-7xl mx-auto px-4 pt-4 mb-8">
        <div class="relative overflow-hidden bg-gradient-to-r from-red-500 via-rose-500 to-red-600 text-white p-4 rounded-2xl shadow-lg shadow-red-500/20 border-t border-white/30 transform transition-all hover:-translate-y-0.5">
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center space-x-3">
                    ...
                    <p class="text-sm md:text-base font-medium tracking-wide text-white/95">
                        <strong class="font-bold text-white">Life-threatening emergency?</strong> Immediate response needed.
                    </p>
                </div>
                <a href="tel:999" class="inline-flex items-center gap-1.5 bg-white text-red-600 font-extrabold px-4 py-2 rounded-xl text-base shadow-md hover:bg-red-50 transition-all hover:scale-105 active:scale-95">
                    <span>📞 Call</span>
    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="lg:hidden mb-8" x-data="{ open: false }">
            <button @click="open = !open" 
                ...
                <svg :class="open ? 'rotate-180 text-rose-600' : 'text-slate-400'" class="w-5 h-5 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:grid lg:grid-cols-3 lg:gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="text-center mb-10">
                    <div class="inline-block mb-2">
                        <span class="px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase bg-indigo-100/80 text-indigo-700 shadow-sm border border-indigo-200/50">
                ...
                @if (!$result)
                    ...
                    <div class="bg-white/90 backdrop-blur-xl rounded-3xl shadow-[0_20px_50px_rgba(8,112,184,0.07)] border border-slate-100/80 p-6 md:p-8 transition-all">
                        <div class="mb-6">
                        ...
                        <div class="mb-6">
                            <span class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Quick Categories</span>
                        ...
                        <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                            ...
                            @else
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200/60 rounded-xl text-sm font-semibold">
                ...
                @else
                    ...
                    <div class="space-y-6">
                        ...
                        @if ($result['type'] === 'crisis')
                            <div class="bg-gradient-to-br from-purple-700 via-indigo-700 to-purple-800 text-white rounded-3xl p-8 text-center shadow-[0_20px_50px_rgba(126,34,206,0.25)] border-t border-white/20">
                                <div class="w-16 h-16 bg-white/10 rounded-2xl backdrop-blur-md flex items-center justify-center text-4xl mx-auto mb-4 shadow-inner">
                                    🤝
                                </div>
                                <h2 class="text-3xl font-black mb-3 tracking-tight">You Are Not Alone</h2>
                                <p class="text-purple-100 mb-6 max-w-lg mx-auto text-base leading-relaxed">{{ $result['message'] }}</p>
                        ...
                        @elseif($result['type'] === 'emergency')
                            <div class="bg-gradient-to-br from-rose-600 via-red-600 to-rose-700 text-white rounded-3xl p-8 text-center shadow-[0_20px_50px_rgba(225,29,72,0.3)] border-t border-white/20">
                                <div class="w-16 h-16 bg-white/10 rounded-2xl backdrop-blur-md flex items-center justify-center text-4xl mx-auto mb-4 shadow-inner animate-bounce">
                                    🚨
                                </div>
                        ...
                        @elseif($result['type'] === 'dispatch' || $result['type'] === 'dispatch_created')
                            <div class="bg-gradient-to-br from-emerald-600 to-teal-700 text-white rounded-3xl p-8 text-center shadow-[0_20px_50px_rgba(13,148,136,0.25)] border-t border-white/20">
                                <div class="w-16 h-16 bg-white/10 rounded-2xl backdrop-blur-md flex items-center justify-center text-4xl mx-auto mb-4 shadow-inner">
                                    🚑
                                </div>
                                ...
                                <p class="text-emerald-200 text-sm mt-3 bg-black/10 inline-block px-4 py-1.5 rounded-full backdrop-blur-sm">A response coordinator will contact you shortly.</p>
                            </div>
                        @elseif($result['type'] === 'facilities')
                            <div class="flex items-center justify-between mb-4">
                                <h2 class="text-xl font-bold text-slate-800">
                                    Nearby Facilities <span class="ml-2 px-2.5 py-0.5 bg-indigo-100 text-indigo-700 text-sm rounded-full">{{ count($result['facilities']) }}</span>
                                </h2>
                            </div>
                            <div class="space-y-4">
                                @foreach ($result['facilities'] as $facility)
                                    <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-slate-100 p-5 hover:shadow-[0_15px_35px_rgba(0,0,0,0.07)] transition-all">
                                        <div class="flex justify-between items-start gap-4">
                                            <div>
                                                <h3 class="font-bold text-lg text-slate-900">{{ $facility['name'] }}</h3>
                                                <p class="text-sm text-slate-500 mt-0.5">{{ $facility['address'] }}</p>
                                                @if (isset($facility['distance']))
                                                    <span class="inline-flex items-center gap-1 text-xs font-semibold text-slate-400 mt-1">
                                        ...
                                        @if (!empty($facility['capabilities']))
                                            <div class="flex flex-wrap gap-1.5 mt-3">
                                                @foreach (array_slice($facility['capabilities'], 0, 5) as $cap)
                                                    <span class="px-2.5 py-1 bg-slate-100 text-slate-600 text-xs font-medium rounded-lg">{{ $cap }}</span>
                                                @endforeach
                                            </div>
                                        @endif

                                        <div class="mt-4 pt-3 border-t border-slate-100 flex flex-wrap gap-3 items-center justify-between">
                                            <div class="flex gap-2">
                                                @if ($facility['phone'])
                                                    <a href="tel:{{ $facility['phone'] }}" class="px-3.5 py-2 bg-indigo-50 text-indigo-700 font-bold text-xs rounded-xl hover:bg-indigo-100 transition-all flex items-center gap-1">
                                                        📞 Call
                                                    </a>
                                                @endif
                                                @if (isset($facility['distance']))
                                                    <a href="https://www.google.com/maps/dir/?api=1&destination={{ urlencode($facility['name']) }}"
                                                        target="_blank" class="px-3.5 py-2 bg-slate-100 text-slate-700 font-bold text-xs rounded-xl hover:bg-slate-200 transition-all flex items-center gap-1">
                                                        🗺️ Directions
                                                    </a>
                                                @endif
                                            </div>
                ...
                <div id="install-prompt"
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center font-black text-xl shadow-md">N</div>
                            <div>
                ...
                <div class="text-center mt-12">
                    <span class="inline-flex items-center gap-1.5 px-4 py-2 bg-white/60 backdrop-blur-md rounded-full text-xs font-medium text-slate-400 border border-slate-100 shadow-sm">
            ...
            <div class="hidden lg:block">
                <div class="sticky top-6 bg-white/80 backdrop-blur-xl rounded-3xl shadow-[0_15px_35px_rgba(0,0,0,0.05)] border border-slate-100/80 p-5">
